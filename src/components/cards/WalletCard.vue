  ...
  <b-col :md="wideCard ? 12 : 6" :style="{ order }" cols="12" class="px-1 mb-2" >
    ...
    <div v-if="!config.IS_TEST_NETWORK" id="changellyModal" :style="{display: changellyModalVisible ? 'block' : 'none'}">
      <div class="changellyModal-content">
        ...
        <iframe
          v-if="changellyModalVisible"
          :src="`https://old.changelly.com/widget/v1?auth=email&from=${changellyCurrency}&to=XLM&merchant_id=dcaa3ae0e64f&address=${data.public_key}&amount=0.1&ref_id=dcaa3ae0e64f&color=00cf70`"
          frameborder="0"
