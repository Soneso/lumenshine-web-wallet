<template>
  <b-col :md="wideCard ? 12 : 6" cols="12" class="px-1 mb-2">
    ...
    <b-modal v-model="fundWalletModalVisible" :title="!config.IS_TEST_NETWORK ? 'Fund Wallet' : 'Fund Wallet via Friendbot'" size="md" hide-footer>
      <div v-if="!config.IS_TEST_NETWORK" class="text-center">
        <div class="py-3">
          <qrcode :options="{ width: 200, margin: 0, color: { light: '#F4F6F2', dark: '#000000' } }" :value="data.public_key" class="bar-code-img"/>
        </div>
        <div class="font-weight-600">Account ID / Public key</div>
        <div class="break-word with-hyphens my-3">
          <public-key :text="data.public_key"/>
        </div>
      </div>
      <div v-else class="text-center"> <!-- Test network -->
        <p class="font-weight-600">Stellar test net public key</p>
